...
Pagin.prototype.find = function (options, _done) {
  function wrapper(done) {
    return function(err, results, _options) {
  ...
  return this._query(options, _done, wrapper);
};
...
Pagin.prototype.findOnly = function (options, _done) {
  function wrapper(done) {
    return function (err, results, _options) {
      if (err) return done(err);
      // TODO error opiton
      if (!results.length) {
        return done(new Error("document does not exist"));
      } else if (results.length !== 1) {
        return done(new Error('duplicate documents'));
      }
      ...
      if (typeof _options.fn !== 'function') {
        ...
        readFile(results[0], mutate(_options.fn, done));
      }
    }
  }
...
Pagin.prototype._query = function (options, done, wrapper) {
